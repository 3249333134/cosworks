import { saveUserRole, profileWithJobs } from './background.js';
import { scheduleRoom, scheduleAccount, takePoolItem, matchesDraw, type DrawWord, type StoryBundle } from './content-pools.js';
...
import { generateUndercoverWords, generateContent } from './content-service.js';
...
export function createApp(store:Store=createStore()){
  ...
  app.get('/api/profile/me',async(req,res,next)=>{try{res.json({ok:true,data:{profile:await profileWithJobs(store,req.auth!.accountId)}});}catch(error){next(error);}});
  app.put('/api/profile/me',async(req,res,next)=>{try{const patch:{displayName?:string;mbti?:Mbti}={};if(req.body.displayName)patch.displayName=String(req.body.displayName).slice(0,24);if(req.body.mbti)patch.mbti=mbtiSchema.parse(req.body.mbti);const profile=await store.updateProfile(req.auth!.accountId,patch);await scheduleAccount(store,req.auth!.accountId);res.json({ok:true,data:{profile}});}catch(error){next(error);}});
  app.post('/api/profile/me/roles',async(req,res,next)=>{try{const input=roleSchema.parse(req.body);const role=await saveUserRole(store,req.auth!.accountId,input);await scheduleAccount(store,req.auth!.accountId);res.status(201).json({ok:true,data:{role}});}catch(error){next(error);}});
  const generateRole:express.RequestHandler=async(req,res,next)=>{try{const input=roleSchema.parse(req.body);const role=await saveUserRole(store,req.auth!.accountId,{...input,...(typeof req.body.id==='string'?{id:req.body.id}:{})},true);await scheduleAccount(store,req.auth!.accountId);res.status(202).json({ok:true,data:{role,task:{id:role.generation!.taskId,status:'queued'}}});}catch(error){next(error);}};
  app.post('/api/roles/generate',generateRole);
  app.post('/api/profile/me/role-generation',generateRole);
  app.get('/api/profile/me/tasks/:id',async(req,res,next)=>{try{const job=await store.getJob(req.params.id);if(!job||job.accountId!==req.auth!.accountId)return res.status(404).json({ok:false,error:'任务不存在'});res.json({ok:true,data:{task:{id:job.id,status:job.status,error:job.error},profile:await profileWithJobs(store,req.auth!.accountId)}});}catch(error){next(error);}});
  app.put('/api/profile/me/roles/:id',async(req,res,next)=>{try{const input=roleSchema.parse(req.body);const role=await saveUserRole(store,req.auth!.accountId,{...input,id:req.params.id});await scheduleAccount(store,req.auth!.accountId);res.json({ok:true,data:{role}});}catch(error){next(error);}});
  app.delete('/api/profile/me/roles/:id',async(req,res,next)=>{try{await store.atomic(async tx=>{await tx.getProfile(req.auth!.accountId);await tx.deleteRole(req.auth!.accountId,req.params.id);});await scheduleAccount(store,req.auth!.accountId);res.json({ok:true});}catch(error){next(error);}});
  ...
  app.post('/api/rooms/:code/rejoin',async(req,res,next)=>{try{const room=await roomForMember(store,req.params.code,req.auth!.accountId);ensureOpen(room);const member=room.members.find(item=>item.accountId===req.auth!.accountId)!;member.online=true;await store.saveRoom(room);await scheduleRoom(store,room.code);broadcast(room.code,{type:'room:member-upsert',member});res.json({ok:true,data:{room:playerSnapshot(room,req.auth!.accountId)}});}catch(error){next(error);}});
  app.post('/api/rooms/:code/join',async(req,res,next)=>{try{const code=roomCodeSchema.parse(req.params.code.toUpperCase());const room=await loadRoom(store,code);if(!room)return res.status(404).json({ok:false,error:'没有找到这个房间'});ensureOpen(room);let member=room.members.find(item=>item.accountId===req.auth!.accountId);if(!member){const user=await store.findUserById(req.auth!.accountId);if(!user)throw new Error('账号不存在');member=memberFor(user.id,user.displayName);room.members.push(member);}member.online=true;await store.saveRoom(room);const profile=await store.getProfile(req.auth!.accountId);const defaultRole=profile.roles.find(role=>role.ipTheme===room.ipTheme&&role.isDefault)??profile.roles.find(role=>role.ipTheme===room.ipTheme)??null;await scheduleRoom(store,room.code);broadcast(code,{type:'room:member-upsert',member});res.json({ok:true,data:{room:playerSnapshot(room,req.auth!.accountId),defaultRole}});}catch(error){next(error);}});
  app.put('/api/rooms/:code/role',async(req,res,next)=>{try{const room=await roomForMember(store,req.params.code,req.auth!.accountId);ensureOpen(room);const profile=await store.getProfile(req.auth!.accountId);const role=profile.roles.find(item=>item.id===req.body.ipRoleId&&item.ipTheme===room.ipTheme);if(!role)return res.status(400).json({ok:false,error:'请选择当前 IP 下的角色'});const member=room.members.find(item=>item.accountId===req.auth!.accountId)!;member.ipRoleId=role.id;member.playerRole=role.name;member.team=String(req.body.team||`${room.ipTheme}队`).slice(0,30);member.ready=Boolean(req.body.ready??true);await store.saveRoom(room);await scheduleRoom(store,room.code);broadcast(room.code,{type:'room:member-upsert',member});res.json({ok:true,data:{room:playerSnapshot(room,req.auth!.accountId)}});}catch(error){next(error);}});
  app.post('/api/rooms/:code/actions',async(req,res,next)=>{try{
    ...
    const room=await store.atomic(async store=>{
      ...
      if(input.gameId==='undercover'&&input.action==='undercover-deal'){
        ...
        const item=await takePoolItem(store,room,'story');applyStoryBundle(room,item.value as StoryBundle,item.source);
        ...
        const challenge=(await takePoolItem(store,room,'dont',req.auth!.accountId)).value as Challenge;replacePlayerChallenge(room,req.auth!.accountId,challenge);
      }else if(input.gameId==='must'&&input.action==='must-forced-change'){
        const profile=await store.getProfile(req.auth!.accountId);const member=room.members.find(item=>item.accountId===req.auth!.accountId);const role=profile.roles.find(item=>item.id===member?.ipRoleId);if(!profile.mbti||!role)throw new Error('请先设置 MBTI 和角色');
        const challenge=(await takePoolItem(store,room,'must',req.auth!.accountId,currentMustContents(room,req.auth!.accountId))).value as Challenge;replacePlayerChallenge(room,req.auth!.accountId,challenge,true);
      }else if(input.gameId==='draw'&&input.action==='draw-guess'){
        const draw=room.game?.privateState?.draw as {word?:string;aliases?:string[]}|undefined;const word=draw?.word??'';const guess=String(input.payload?.guess||'').trim();
