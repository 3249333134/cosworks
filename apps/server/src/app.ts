...
app.post('/api/roles/generate',requireAuth,async(req,res,next)=>{
  ...
  app.post('/api/rooms/:code/host-actions',async(req,res,next)=>{try{
    ...
    if(action==='set-plan'){
      ...
      if(room.game&&room.game.phase!=='settled')throw new Error('当前游戏还在进行，请使用“提前开始下一场”');const planned=room.gamePlan[room.planCursor];const gameId=String(req.body.gameId||planned?.gameId||'') as GameId;if(!GAME_IDS.includes(gameId))throw new Error('请先编排至少一场游戏');if(planned&&gameId!==planned.gameId)throw new Error('请按聚会时间线开始下一场游戏');await prepareAndStartGame(store,room,gameId,planned?.id??null,options);if(planned){const total=planned.rounds??1;if(room.planRound<total)room.planRound++;else{room.planRound=1;room.planCursor=Math.min(room.planCursor+1,room.gamePlan.length);}}
