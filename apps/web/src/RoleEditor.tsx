...
import { avatarBackground, isAvatarColor, roleAvatarColor } from './avatar-colors';
...
const fields=['ipTheme','name','personaTags','quote','signatureAction','ability','isDefault','avatarColor'] as const;
...
export function RoleEditor({role,onSaved,onCancel,onGenerated}:{role:RoleDraft;onSaved:(role:IpRole)=>void;onCancel?:()=>void;onGenerated?:(role:IpRole)=>void}){
  const [draft,setDraft]=useState<RoleDraft>(()=>({...role,avatarColor:avatarBackground(role.avatarColor,roleAvatarColor(role.id))})),[pending,setPending]=useState(false),[message,setMessage]=useState('');
  ...
  const submit=async(generate:boolean)=>{
    if(requestPending.current||!isAvatarColor(draft.avatarColor??''))return;requestPending.current=true;setPending(true);setMessage('');const submitted={...current.current};
    try{
  ...
  return <form className="settings-role-editor" aria-label="编辑角色" onSubmit={e=>{e.preventDefault();void submit(false);}}><fieldset disabled={pending}>
    ...
    <AvatarColorPicker label="角色头像颜色" name={draft.name} value={draft.avatarColor??roleAvatarColor(draft.id)} disabled={pending} onChange={avatarColor=>update({avatarColor})}/>
    <button type="button" className="ai-generate-btn" disabled={pending||!isAvatarColor(draft.avatarColor??'')||running||!draft.ipTheme.trim()||!draft.name.trim()} onClick={()=>void submit(true)}><span>{pending?'正在保存…':running?'资料补全中…':draft.generation?.status==='failed'?'重试 AI 补全':'AI 生成'}</span><span className="ai-hint">先保存角色，再查找资料自动补全</span></button>
    ...
    <button className="primary" disabled={pending||!isAvatarColor(draft.avatarColor??'')||!draft.ipTheme.trim()||!draft.name.trim()}>{pending?'正在保存…':'保存角色'}</button>
